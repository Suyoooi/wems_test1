...
function GrpModal({
  ...
  let propInfoUrl = `/api/setting/topic/propertyApi`;
  ...
  const fetchTableData = async () => {
    try {
      const bodyData = {
        ...
        raw_topic_sn: propInfo.raw_topic_sn,
      };
      ...
      const topicPropType = dataInfo.topicPropType;
      const topicPropValue = dataInfo.topicPropChangeable;
      const detailDataVal: any = [];

      var keys = Object.keys(topicPropValue);
      for (var i = 0; i < keys.length; i++) {
        var key = keys[i];
        let concatVal: any = {};
        concatVal.key = key;
        concatVal.type = topicPropType[key];
        concatVal.value = topicPropValue[key];
    ...
    if (tableData != "") {
      ...
      if (newValArr && newValArr != "") {
        const bodyData = {
          ...
          topic_name: propInfo.ems_tpc_nm,
          topic_prop_value_list: newValArr,
        };
