...
function Modal(
  ...
  const [pUserNm, setPUserNm] = useState("");
  ...
  const fetchTableData = async () => {
    try {
      ...
      const userNm = propInfo.user_name;
  ...
  const fnAddProp = async () => {
    try {
      if (confirm(t("MUL_ST_0050") as string)) {
        const bodyData = {
          case_method: "PROP_POST",
          tib_srvr_sn: propInfo.tib_srvr_sn,
          principal: propInfo.user_name,
          grant_list: checked,
        };
