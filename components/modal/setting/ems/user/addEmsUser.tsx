...
function Modal({
  ...
  } = useForm<AddEmsUserInputBody>({
    resolver: yupResolver(schema),
  });

  const handleRegistration: SubmitHandler<AddEmsUserInputBody> = (data) => {
    addUserList(data);
  };
  ...
  const [userTableData, setUserTableData] = useState<EMSSrvrListData[]>([]);
  ...
  const { inUserNm, inPwd, inDesc } = inputData;
  ...
  async function addUserList(dataVal: any) {
    ...
    if (filterData.length > 0) {
      ...
      tableInfo.addRow(
        {
          user_nm: dataVal.inUserNm,
          user_pwd: dataVal.inPwd,
          user_desc: dataVal.inDesc,
        },
        true
      );
    }
  ...
  const userInfoCreate = async () => {
    const tableInfo: any = tableRef.current;
    const userTableInfo: any = userTableRef.current;

    const selectedData = tableInfo.table.getSelectedData();
    const addUserData = userTableInfo.table.getData();

    const srvrSnData: any = [];
    const userListData: any = [];

    if (selectedData != "") {
      if (addUserData && addUserData != "") {
        if (confirm(t("MUL_ST_00169") as string)) {
          selectedData.forEach((sData: any, idx: number) => {
            srvrSnData.push(sData.tib_srvr_sn);
          });

          addUserData.forEach((uData: any, idx: number) => {
            const concatVal: any = {};
            concatVal.username = uData.user_nm;
            concatVal.password = uData.user_pwd;
            concatVal.description = uData.user_desc;
            userListData.push(concatVal);
          });

          const bodyData = {
            case_method: "USER_POST",
            tib_srvr_sn: srvrSnData,
            tibco_user_info_list: userListData,
          };
          ...
          if (data && data.code == "200") {
            ...
            if (dataVal && dataVal.includes("실패")) {
              alert(t("MUL_ST_00225"));
            } else {
              alert(t("MUL_ST_0088"));
