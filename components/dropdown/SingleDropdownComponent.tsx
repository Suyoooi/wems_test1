...
interface DropdownComponentProps {
  onServerSelected: (tibSrvrSn: number[]) => void;
  onSelectedServerName: (alias: string[]) => void;
}

const SingleDropdownComponent: React.FC<DropdownComponentProps> = ({
  onServerSelected,
    ...
    if (selectedCount > 0) {
      toast.success(
        <div>
          <p>{confirmVal}</p>
          <div>
            {t("MUL_ST_0069")} {`${selectedServerNames.join(", ")}`}
          </div>
        </div>,
        toastOptions
      );
