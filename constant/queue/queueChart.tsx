...
import { QUEUE_DATE_COLUMNS_LIST } from "../data/QueueListData";
...
const QueueChart: React.FC<QueueChartProps> = () => {
  // 언어 선택
  ...
  const fetchTableData = async () => {
    ...
    try {
      ...
      const clctDtArraySet = new Set(
        Data.chart_dto.map((item: any) => item.chart_info[0].clct_dt)
      );
      ...
      const queueNameSet = new Set(
        Data.chart_dto.map((item: any) => item.ems_nm)
      );
      const queueName: any = Array.from(queueNameSet);
      setQueueList(queueName);

      const arrLabelInfo: any[] = Array.from(queueNameSet);
      ...
      Data.chart_dto.forEach((item: any) => {
        arrLabelInfo.forEach((arrItem: any, idx: number) => {
          const queueNm = item.ems_nm;
          ...
          if (queueNm == labelNm) {
            ...
            // const formattedDate = moment(x).format("YYYY-MM-DD HH:mm:ss");
    ...
    <>
      {isOpenModal && (
        <ModalChart
          ...
          columns={columns}
          excelColumns={QUEUE_HIST_LIST_EXCEL}
          excelFileNm="Queue_History"
